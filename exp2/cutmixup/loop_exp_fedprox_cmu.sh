...
for DATASET in "${data_sets[@]}"; do
    ...
    for SPLIT_MODE in "iid" "dirichlet"; do
        ...
        if [ "$SPLIT_MODE" = "iid" ]; then
            ...
            ALPHA=0.6
            EXP_NAME=FedProx_cm"$CM_PROB"_"$MU_PROB"_iid
            python federated_train.py client=Prox server=base visible_devices=\'$DEVICE\' \
            ...
            for ALPHA in "${alpha_values[@]}"; do
                EXP_NAME=FedProx_cm"$CM_PROB"_"$MU_PROB"_"$ALPHA"
                python federated_train.py client=Prox server=base visible_devices=\'$DEVICE\' \
                    exp_name="$EXP_NAME" dataset="$DATASET" trainer.num_clients=100 \
