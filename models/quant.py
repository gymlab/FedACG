...
class quantization(nn.Module):
    def __init__(self, args=None, tag='fm', shape=[], feature_stride=None, logger=None, groups=1):
        ...
        self.args = args 
        # print(f'ID QUANTIZATION {id(args)}')
        # print(f'ID QUANTIZATION {id(self.args)}')
        
        self.logger = logger 
        self.index = -1
        self.tag = tag # 양자화 대상 ex) 'fm' : feature map , 'wt' : weights
        self.method = 'none' 
        self.choice = 'none'
        if logger is None:
        ...
        if self.num_levels is None or self.num_levels <= 0:
            self.num_levels = int(2 ** self.bit) # 양자화는 2^bit로 계산
...
# 
    ...
    def quantization_value(self, x, y):
        # print(f'afdfsafdsafd{id(self.args)}')
        if self.iteration.data <= self.stable:
    ...
    def forward(self, x):
        ...
        if self.method == 'dorefa':
            if self.tag in ['fm', 'ot']:
                ...
                if 'lsq' in self.args.quantizer.keyword or 'wt_lsq' in self.args.quantizer.keyword:
                    ...
                    if 'symmetry' in self.args.quantizer.keyword: # 음수와 양수 범위를 동일하게 유지
                        ...
                        y = self.clamp(y, min=-1, max=1)
                        y = (y + 1.0) / 2.0
                        y = self.quant.apply(y, self.level_num.item() - 1) # 0과 1사이의 값
                        
                        # quant_y = y
                        # quant_y = quant_y.reshape(c1,c2, kh, kw)
            ...
            self.iteration.data = self.iteration.data + 1
            # print(f'In forward ID {id(self.args)}')
            return self.quantization_value(x, y)
...
class custom_conv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
        ...
        # print(f"self.args : {id(self.args)}")
        # print(f"args : {id(args)}")
    ...
    def forward(self, inputs):
        if not self.force_fp and self.training:
...
class quant_conv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
            args=None, force_fp=False, feature_stride=1, bits_weights=32, bits_activations=32):
        super(quant_conv, self).__init__(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.args = args
    ...
    def forward(self, inputs):
        weight = self.quant_weight(inputs)
        
        return weight

class quant_linear(nn.Linear):
    def __init__(self, in_channels, out_channels, dropout=0, bias=True, args=None):
        super(quant_linear, self).__init__(in_channels, out_channels, bias=bias)
        self.args = args
        self.dropout = dropout
        self.force_fp = True
        if self.args.quantizer is not None and hasattr(self.args.quantizer, 'keyword'):
