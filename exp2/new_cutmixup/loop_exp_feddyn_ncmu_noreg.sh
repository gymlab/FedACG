...
for DATASET in "${data_sets[@]}"; do
    ...
    for SPLIT_MODE in "dirichlet"; do
        ...
        if [ "$SPLIT_MODE" = "iid" ]; then
            ...
            ALPHA=0.6
            EXP_NAME=FedDyn_ncmu"$CM_PROB"_"$MU_PROB"_iid_no_reg
            python federated_train.py client=Dyn server=FedDyn visible_devices=\'$DEVICE\' \
                exp_name="$EXP_NAME" dataset="$DATASET" trainer.num_clients=100 \
                split.mode="$SPLIT_MODE" trainer.participation_rate=0.05 \
                dataset.new_cutmixup.use=true dataset.new_cutmixup.use_reg=falsen \
            ...
            for ALPHA in "${alpha_values[@]}"; do
                EXP_NAME=FedDyn_ncmu"$CM_PROB"_"$MU_PROB"_"$ALPHA"_no_reg
                python federated_train.py client=Dyn server=FedDyn visible_devices=\'$DEVICE\' \
                    exp_name="$EXP_NAME" dataset="$DATASET" trainer.num_clients=100 \
