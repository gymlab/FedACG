...
class ResNet_Conv_LPT(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, l2_norm=False, use_pretrained=False, use_bn_layer=False,
        ...
        super(ResNet_Conv_LPT, self).__init__()
    ...
    def update_all_global_std(self, momentum=0.1):
        """모델 내부의 모든 nn.Conv2d 레이어에서 update_global_std() 호출"""
        for module in self.modules():  # self.modules()를 사용하면 모델의 모든 서브모듈을 가져옴
            if isinstance(module, nn.Conv2d):  # WSConv2d인 경우
...
class ResNet_WS_LPT(ResNet_Conv_LPT):
...
@ENCODER_REGISTRY.register()
class ResNet34_LPT(ResNet_WS_LPT):

    def __init__(self, args: DictConfig, num_classes: int = 10, **kwargs):
        super().__init__(BasicBlock_LPT, [3, 4, 6, 3], num_classes=num_classes, **kwargs
                        #  l2_norm=args.model.l2_norm,
                        #  use_pretrained=args.model.pretrained, use_bn_layer=args.model.use_bn_layer
                         )
     
@ENCODER_REGISTRY.register()
class ResNet8_LPT(ResNet_WS_LPT):

    def __init__(self, args: DictConfig, num_classes: int = 10, **kwargs):
        super().__init__(BasicBlock_LPT, [1, 1, 1, 1], num_classes=num_classes, **kwargs
                        #  l2_norm=args.model.l2_norm,
                        #  use_pretrained=args.model.pretrained, use_bn_layer=args.model.use_bn_layer
                         )