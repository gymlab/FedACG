...
class EfficientNetV2(nn.Module):
    ...
    def __init__(self, layer_infos, out_channels=1280, nclass=0, dropout=0.2, stochastic_depth=0.0,
        ...
        self.stem = ConvBNAct(3, self.in_channel, 3, 1, 1, self.norm_layer, self.act)
...
class EfficientNet_v2(EfficientNetV2):    
    def __init__(self, args: DictConfig, num_classes: int = 10, **kwargs):

        # 1. 모델 이름을 통해 구조 설정값(튜플 리스트)을 가져옵니다.
        structure_config = get_efficientnet_v2_structure(args.model.type)
        
        # 2. 튜플 리스트를 MBConvConfig 객체 리스트로 변환합니다. (매우 중요!)
        # EfficientNetV2는 입력으로 MBConvConfig 객체들을 기대하기 때문입니다.
        layer_infos = [MBConvConfig(*layer_config) for layer_config in structure_config]
        
        # 3. 부모 클래스(EfficientNetV2) 초기화
        super().__init__(layer_infos=layer_infos, nclass=num_classes, out_channels=256)
        
        # 4. 가중치 초기화 적용
        efficientnet_v2_init(self)