...
data_sets=(tinyimagenet cifar100)
alpha_values=(0.3)
seeds=(0)
CM_PROB=0.15
MU_PROB=0.15
        ...
        for DROP in "${DROPS[@]}"; do
            ...
            for SPLIT_MODE in "dirichlet"; do
                ...
                if [ "$SPLIT_MODE" = "iid" ]; then
                    ...
                    ALPHA=0.6
                    EXP_NAME=FedAvg_rmsda"$CM_PROB"_"$MU_PROB"_iid_seed"$seed"_Swin_D"$DROP"
                    python federated_train.py client=base server=base visible_devices=\'$DEVICE\' \
                        ...
                        batch_size="$BATCH_SIZE" wandb=True project="CVPR_REBUTTAL" seed=$seed model=Swin model.drop_prob="$DROP"
                else
                    # For non-iid mode, iterate over alpha values
                    for ALPHA in "${alpha_values[@]}"; do
                        EXP_NAME=FedAvg_rmsda"$CM_PROB"_"$MU_PROB"_"$ALPHA"_seed"$seed"_Swin_D"$DROP"
                        python federated_train.py client=base server=base visible_devices=\'$DEVICE\' \
                            exp_name="$EXP_NAME" dataset="$DATASET" trainer.num_clients=100 \
